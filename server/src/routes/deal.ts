...
dealRouter.put("/:id", authMiddleware, async (req: AuthRequest, res) => {
  ...
  try {
    ...
    if (
      existingDeal.customer.createdById !== userId &&
      existingDeal.assignedToId !== userId
    ) {
      return res
        .status(403)
        .json({ message: "Not authorized to update this deal" });
    }
...
dealRouter.get("/", authMiddleware, async (req: AuthRequest, res) => {
  const userId = req.userId;
